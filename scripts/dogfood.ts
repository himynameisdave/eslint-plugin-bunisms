...
const { default: plugin }: { default: typeof sourcePlugin } = await import(
...
for (const directory of process.argv.slice(2)) {
  ...
  for (const file of listing.stdout.toString().trim().split('\n')) {
    // oxlint-disable-next-line no-await-in-loop -- Bound memory usage when scanning large repositories.
    const [result] = await eslint.lintText(await Bun.file(file).text(), {
      filePath: `dogfood${extname(file)}`,
    });
