...
export interface BuiltinReference { module: string; path: string[] }
...
export function resolveBuiltin(context: Rule.RuleContext, node: Expression | Node): BuiltinReference | undefined {
  if (node.type === "MemberExpression") {
    const property = propertyName(node.property, node.computed);
    const object = resolveBuiltin(context, node.object);
    if (property && object) return { ...object, path: [...object.path, property] };
    return;
  }
  if (node.type === "CallExpression" && node.callee.type === "Identifier" && node.callee.name === "require") {
    const variable = variableFor(context, node.callee);
    if (variable?.defs.length || node.arguments.length !== 1) return;
  ...
  if (!variable || variable.defs.length !== 1 || isModified(variable)) return;
  const definition = variable.defs[0]!;
  if (definition.type === "ImportBinding") {
    const declaration = definition.parent;
    // Type-only imports cannot identify a runtime API.
    if ((declaration as Node & { importKind?: string }).importKind === "type"
      || (definition.node as Node & { importKind?: string }).importKind === "type") return;
  ...
  if (!origin) return;
  if (id.type === "Identifier") return origin;
  if (id.type === "ObjectPattern") {
    const property = id.properties.find((entry) => entry.type === "Property"
      && entry.value.type === "Identifier" && entry.value.name === node.name);
    if (property?.type !== "Property") return;
    const name = propertyName(property.key, property.computed);
    if (name) return { ...origin, path: [...origin.path, name] };
  }
}
